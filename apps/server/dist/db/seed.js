...
dotenv_1.default.config({ path: path_1.default.resolve(__dirname, "../../.env") }); // Now __dirname is defined
...
async function seedDatabase() {
    ...
    const dataPath = path_1.default.join(__dirname, "../data/nigeria-states-lgas.json");
    let stateLgaData;
    try {
    ...
    for (const stateName in stateLgaData) {
        ...
        if (!stateCode) {
            console.warn(`Warning: No ISO code found for state: ${stateName}. Skipping.`);
            continue;
        }
    ...
    if (statesToInsert.length > 0) {
        try {
            ...
            await db.insert(schema_js_1.states).values(statesToInsert).onConflictDoNothing();
    ...
    for (const stateName in stateLgaData) {
        ...
        if (!lgaList || !Array.isArray(lgaList)) {
            console.warn(`Warning: Invalid or missing LGA list for state: ${stateName}. Skipping.`);
            continue;
        }
        lgaCounter[stateCode] = 1; // Reset counter for each state
        for (const lgaName of lgaList) {
            if (!lgaName || typeof lgaName !== "string") {
                console.warn(`Warning: Invalid LGA name found in ${stateName}: ${lgaName}. Skipping.`);
                continue;
            }
    ...
    const existingCategories = await db.select().from(schema_js_1.addressCategories);
    if (existingCategories.length === 0) {
        ...
        try {
            // Insert categories
            const result = await db.insert(schema_js_1.addressCategories).values(categories).returning();
