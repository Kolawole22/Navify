...
exports.testRuralAddressing = testRuralAddressing;
exports.testRuralScenarios = testRuralScenarios;
exports.runTests = runTests;
const rural_addressing_1 = require("../utils/rural-addressing");
...
async function testRuralAddressing() {
    ...
    for (const location of testLocations) {
        ...
        const coordDesc = (0, rural_addressing_1.generateCoordinateDescription)(location.latitude, location.longitude, location.city);
        console.log(`   ${coordDesc}\n`);
        // Test 2: Landmark-based address
        console.log("2️⃣ Landmark-based Address:");
        const landmarkAddr = (0, rural_addressing_1.generateLandmarkBasedAddress)({
        ...
        const directionAddr = (0, rural_addressing_1.generateDirectionBasedAddress)({
        ...
        const villageAddr = (0, rural_addressing_1.generateVillageAreaAddress)({
        ...
        sampleInputs.forEach((input, index) => {
            const parsed = (0, rural_addressing_1.parseRuralAddressInput)(input);
        ...
        try {
            const ruralComponents = await (0, rural_addressing_1.generateRuralAddressComponents)(location.latitude, location.longitude, location.city, "Near the community health center");
            console.log(`   Primary: ${ruralComponents.primaryAddress}`);
            console.log(`   Alternatives:`);
            ruralComponents.alternativeAddresses.forEach((alt, i) => {
                console.log(`     ${i + 1}. ${alt}`);
            });
            console.log(`   Landmark Suggestions: ${ruralComponents.suggestedComponents.landmarks
                .slice(0, 5)
                .join(", ")}`);
            console.log(`   Traditional Names: ${ruralComponents.suggestedComponents.traditional
                .slice(0, 3)
                .join(", ")}`);
        }
        catch (error) {
...
async function testRuralScenarios() {
    ...
    for (const scenario of scenarios) {
        ...
        try {
            const result = await (0, rural_addressing_1.generateRuralAddressComponents)(scenario.latitude, scenario.longitude, scenario.city, scenario.userInput);
...
//# sourceMappingURL=test-rural-addressing.js.map