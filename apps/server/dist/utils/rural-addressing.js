...
exports.RuralAddressType = void 0;
exports.generateLandmarkBasedAddress = generateLandmarkBasedAddress;
exports.generateDirectionBasedAddress = generateDirectionBasedAddress;
exports.generateVillageAreaAddress = generateVillageAreaAddress;
exports.generateCoordinateDescription = generateCoordinateDescription;
exports.generateRuralAddressSuggestions = generateRuralAddressSuggestions;
exports.parseRuralAddressInput = parseRuralAddressInput;
exports.enhanceRuralAddress = enhanceRuralAddress;
exports.findNearbyAddresses = findNearbyAddresses;
exports.generateRuralAddressComponents = generateRuralAddressComponents;
const db_1 = require("../db");
const schema_1 = require("../db/schema");
const drizzle_orm_1 = require("drizzle-orm");
// --- Rural Address Types ---
var RuralAddressType;
(function (RuralAddressType) {
    RuralAddressType["LANDMARK_BASED"] = "landmark";
    RuralAddressType["DIRECTION_BASED"] = "direction";
    RuralAddressType["VILLAGE_AREA"] = "village";
    RuralAddressType["TRADITIONAL_NAME"] = "traditional";
    RuralAddressType["COORDINATE_DESCRIPTION"] = "coordinate";
    RuralAddressType["CLOSEST_KNOWN_POINT"] = "closest_point";
})(RuralAddressType || (exports.RuralAddressType = RuralAddressType = {}));
// --- Rural Address Generation Strategies ---
/**
 * Generate a descriptive address using landmarks and directions
 */
function generateLandmarkBasedAddress(options) {
    const { primaryLandmark, secondaryLandmark, direction, distance, description, } = options;
 ...
 */
function generateDirectionBasedAddress(options) {
 ...
 */
function generateVillageAreaAddress(options) {
 ...
 */
function generateCoordinateDescription(latitude, longitude, nearestTown) {
 ...
 */
function generateRuralAddressSuggestions(city, landmark) {
 ...
 */
function parseRuralAddressInput(input) {
    const inputLower = input.toLowerCase();
    // Landmark-based patterns
    if (inputLower.includes("near") ||
    ...
    if (inputLower.includes("village") ||
 ...
 */
async function enhanceRuralAddress(originalAddress, latitude, longitude, city) {
 ...
 */
async function findNearbyAddresses(latitude, longitude, radiusKm = 5) {
    try {
        // Using Haversine formula to find nearby addresses
        const nearbyAddresses = await db_1.db
            .select({
        ...
        return nearbyAddresses.map((addr) => {
            ...
            if (addr.specialDescription)
                addressStr += addr.specialDescription + ", ";
 ...
 */
async function generateRuralAddressComponents(latitude, longitude, city, userInput) {
    const suggestions = generateRuralAddressSuggestions(city);
    const nearbyAddresses = await findNearbyAddresses(latitude, longitude);
    const coordinateDescription = generateCoordinateDescription(latitude, longitude, city);
    let primaryAddress = userInput || coordinateDescription;
    if (userInput) {
        const enhanced = await enhanceRuralAddress(userInput, latitude, longitude, city);
...
//# sourceMappingURL=rural-addressing.js.map